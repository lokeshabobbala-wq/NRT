...
def fetch_execution_time_window(rds_cursor, reportregion):
    ...
    try:
        ...
        return join_start, join_end, batch_run_date
    except Exception as e:
...
def check_bmt_report_completion(rds_cursor, reportregion, batch_run_date, join_start, rds_connection, current_time):
    """
...
def trigger_glue_job(glue_client, rds_cursor, rds_connection, reportregion, batch_run_date, glue_job):
    ...
    try:
        ...
        rds_cursor.execute(
            ...
            AND regionname = %s 
            AND report_source = %s;""",
            (d, 'Submitted', 'NA', batch_run_date, reportregion, 'SPDST')
        )
        rds_connection.commit()
    ...
    try:
        ...
        if current_time > join_start:
            rds_cursor.execute(
                ...
                AND regionname = %s 
                AND report_source = %s;""",
                (error_msg, 'Delay', batch_run_date, reportregion, 'SPDST')
            )
            rds_connection.commit()
...
def update_pending_status(rds_cursor, rds_connection, reportregion, batch_run_date, filesnotreceived):
    ...
    """
    try:
        print("Not all conditions met. Updating pending status...")
        ...
        rds_cursor.execute(
            ...
            AND regionname = %s 
            AND report_source = %s;""",
            ('Delay', error_msg, batch_run_date, reportregion, 'SPDST')
        )
        rds_connection.commit()
    except Exception as e:
...
def create_initial_log_entry(rds_cursor, rds_connection, batch_run_date, reportregion, glue_job, join_start, join_end):
    ...
    try:
        ...
        rds_cursor.execute(
            ...
            (batch_run_date, reportregion, execution_status, glue_job, 'SPDST', join_start, join_end, error_message)
        )
        rds_connection.commit()
...
def lambda_handler(event, context):
    ...
    try:
        ...
        cutoff_strt_hour = int(os.environ['cutoff_strt_hour'])
        cutoff_strt_min = int(os.environ['cutoff_strt_minute'])
        cutoff_end_hour = int(os.environ['cutoff_end_hour'])
        cutoff_end_min = int(os.environ['cutoff_end_minute'])
        delay_cutoff_strt_hour = int(os.environ['delay_cutoff_strt_hour'])
        delay_cutoff_strt_min = int(os.environ['delay_cutoff_strt_minute'])
        delay_cutoff_end_hour = int(os.environ['delay_cutoff_end_hour'])
        delay_cutoff_end_min = int(os.environ['delay_cutoff_end_minute'])
        ...
        redshift_connection = get_db_connection(redshift_secret_name)
        rds_connection = get_db_connection(rds_secret_name)
        ...
        join_start, join_end, batch_run_date = fetch_execution_time_window(rds_cursor, reportregion)
        
        # Check existing execution status
        execution_exists, execution_status = check_existing_execution_status(rds_cursor, reportregion, batch_run_date)
        
        if execution_exists:
            # Handle duplicate execution scenarios
            if handle_duplicate_execution(glue_client, rds_cursor, rds_connection, reportregion, batch_run_date, execution_status, current_time, join_start):
            ...
            snstimeperiod = determine_time_period_status(current_time, cutoff_strt_hour, cutoff_strt_min, cutoff_end_hour, cutoff_end_min)
            
            # Check BMT report completion
            BMTFlag = check_bmt_report_completion(rds_cursor, reportregion, batch_run_date, join_start, rds_connection, current_time)
